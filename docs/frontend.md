...
- `VITE_API_URL` — переменная окружения, которая позволяет указать другой префикс (например, полный URL внешнего бэкенда).
- `ENDPOINTS` — константы с путями внешнего API.
...
- Запросы `/api/*` исключены из fallback.
