<template>
  <div>
    <div class="flex items-center justify-between border-b border-white/10 px-4 py-2">
      <h1 class="section-title text-lg">Коллекции</h1>
    </div>
    <div v-if="isLoading" class="flex min-h-[50vh] items-center justify-center">
      <Spinner />
    </div>
    <div v-else-if="content.length">
      <CollectionSection title="Коллекции" :content="content" :is-my-collections="isMyCollections" />
      <button
        v-if="hasMore"
...
const isMyCollections = computed(() => {
  return userStore.user?.id === profileId.value;
});
...
async function fetchPage(reset = false) {
  ...
  let url = `${ENDPOINTS.collection.userCollections}/${profileId.value}/${page.value}`;
  if (userStore.token) {
